...
func TestVlogBase(t *testing.T) {
	...
	require.True(t, int64(len(val1)) >= database.opt.ValueThreshold)
	...
	// 关闭会调的锁
	defer db.RunCallback(log.getUnlockCallback(lf1))
	defer db.RunCallback((log.getUnlockCallback(lf2)))
	...
	// 比较entry对象是否相等
	readEntries := []db.Entry{*e1, *e2}
	require.EqualValues(t, []db.Entry{
		{
			Key:    []byte("samplekey"),
			Value:  []byte(val1),
			Meta:   utils.BitValuePointer,
...
func newRandEntry(sz int) *db.Entry {
	v := make([]byte, sz)
...
func getItemValue(t *testing.T, item *db.Entry) (val []byte) {
	t.Helper()
	if item == nil {
		return nil
	}
