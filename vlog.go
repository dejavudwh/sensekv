...
var lfDiscardStatsKey = []byte("!sensekv!discard") // For storing lfDiscardStats
...
func (vlog *valueLog) newValuePtr(e *db.Entry) (*db.ValuePtr, error) {
	// TODO 尝试使用对象复用，后面entry对象也应该使用
	req := requestPool.Get().(*request)
	req.reset()
	req.Entries = []*db.Entry{e}
	req.Wg.Add(1)
...
func (vlog *valueLog) open(database *DB, ptr *db.ValuePtr, replayFn db.LogEntry) error {
	...
	vlog.db.vhead = &db.ValuePtr{Fid: vlog.maxFid, Offset: uint32(lastOffset)}
...
func (vlog *valueLog) read(vp *db.ValuePtr) ([]byte, func(), error) {
...
func (vlog *valueLog) write(reqs []*request) error {
	...
	for i := range reqs {
		...
		for j := range b.Entries {
			e := b.Entries[j]
			// 在set的时候已经判断过 但是gc时没有判断过所以需要重新判断
			if vlog.db.shouldWriteValueToLSM(e) {
				// 没做kv分离 只做个占位
				b.Ptrs = append(b.Ptrs, &db.ValuePtr{})
				continue
			}
			var p db.ValuePtr
...
func (vlog *valueLog) runGC(discardRatio float64, head *db.ValuePtr) error {
	select {
...
func (vlog *valueLog) rewrite(f *file.LogFile) error {
	...
	wb := make([]*db.Entry, 0, 1000)
	var size int64

	var count, moved int
	fe := func(e *db.Entry) error {
		count++
		if count%100000 == 0 {
	...
	_, err := vlog.iterate(f, 0, func(e *db.Entry, vp *db.ValuePtr) error {
		return fe(e)
	})
	if err != nil {
		return err
...
// logFile unlocking.
func (vlog *valueLog) readValueBytes(vp *db.ValuePtr) ([]byte, *file.LogFile, error) {
...
// (if non-nil)
func (vlog *valueLog) getFileRLocked(vp *db.ValuePtr) (*file.LogFile, error) {
	vlog.filesLock.RLock()
...
func (vlog *valueLog) replayLog(lf *file.LogFile, offset uint32, replayFn db.LogEntry) error {
...
func (vlog *valueLog) iterate(lf *file.LogFile, offset uint32, fn db.LogEntry) (uint32, error) {
	if offset == 0 {
...
func (r *safeRead) Entry(reader io.Reader) (*db.Entry, error) {
...
// initVLog
func (database *DB) initVLog() {
	// 为了vlog的重放
	// checkpoint
	vp, _ := database.getHead()
	vlog := &valueLog{
		dirPath: database.opt.WorkDir,
		// 延迟删除
	...
	vlog.db = database
	vlog.opt = *database.opt
	// gc协程调度
	vlog.garbageCh = make(chan struct{}, 1)
	if err := vlog.open(database, vp, database.replayFunction()); err != nil {
		utils.Panic(err)
	}
	database.vlog = vlog
}
...
func (database *DB) getHead() (*db.ValuePtr, uint64) {
	var vptr db.ValuePtr
	return &vptr, 0
}
func (database *DB) replayFunction() func(*db.Entry, *db.ValuePtr) error {
	toLSM := func(k []byte, vs db.ValueStruct) {
		database.lsm.Set(&db.Entry{
			Key:       k,
			Value:     vs.Value,
	...
	return func(e *db.Entry, vp *db.ValuePtr) error { // Function for replaying.
		nk := make([]byte, len(e.Key))
		copy(nk, e.Key)
		var nv []byte
		meta := e.Meta
		if database.shouldWriteValueToLSM(e) {
		...
		// 快照
		database.updateHead([]*db.ValuePtr{vp})
...
func (database *DB) updateHead(ptrs []*db.ValuePtr) {
	var ptr *db.ValuePtr
	...
	utils.CondPanic(ptr.Less(database.vhead), fmt.Errorf("ptr.Less(db.vhead) is true"))
	database.vhead = ptr
}
...
// Set
func (v *valueLog) set(entry *db.Entry) error {
	return nil
}

func (v *valueLog) get(entry *db.Entry) (*db.Entry, error) {
...
// GC 部分
// 选择需要gc的log文件
func (vlog *valueLog) pickLog(head *db.ValuePtr) (files []*file.LogFile) {
	vlog.filesLock.RLock()
	defer vlog.filesLock.RUnlock()
	fids := vlog.sortedFids()
	switch {
...
func (vlog *valueLog) sample(samp *sampler, discardRatio float64) (*reason, error) {
	...
	// 重放遍历vlog文件
	_, err = vlog.iterate(samp.lf, 0, func(e *db.Entry, vp *db.ValuePtr) error {
		numIterations++
