...
func (lsm *LSM) NewIterators(opt *db.Options) []db.Iterator {
	iter := &Iterator{}
	iter.iters = make([]db.Iterator, 0)
...
func (iter *Iterator) Item() db.Item {
...
type memIterator struct {
	innerIter db.Iterator
}

func (m *memTable) NewIterator(opt *db.Options) db.Iterator {
...
func (iter *memIterator) Item() db.Item {
...
func (lm *levelManager) NewIterators(options *db.Options) []db.Iterator {
	return lm.iterators()
}
...
func (iter *levelIterator) Item() db.Item {
	return &Item{}
}
...
/* ConcatIterator links table arrays into one iterator, making iteration more efficient */
type ConcatIterator struct {
	idx     int // Which iterator is active now.
	cur     db.Iterator
	iters   []db.Iterator // Corresponds to tables.
	tables  []*table      // Disregarding reversed, this is in ascending order.
	options *db.Options   // Valid options are REVERSED and NOCACHE.
}

func NewConcatIterator(tbls []*table, opt *db.Options) *ConcatIterator {
	iters := make([]db.Iterator, len(tbls))
...
func (s *ConcatIterator) Item() db.Item {
	return s.cur.Item()
}
...
func (n *node) setIterator(iter db.Iterator) {
	n.iter = iter
...
func (mi *MergeIterator) Next() {
	// Ignoring equal keys is equivalent to deleting
	// Skip all equal keys with a for loop
	for mi.Valid() {
...
func (mi *MergeIterator) Item() db.Item {
...
func NewMergeIterator(iters []db.Iterator, reverse bool) db.Iterator {
	switch len(iters) {
	case 0:
		return &Iterator{}
	case 1:
