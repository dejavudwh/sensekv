...
func (lf *LogFile) Read(p *db.ValuePtr) (buf []byte, err error) {
	offset := p.Offset
...
// +--------+-----+-------+-------+
func (lf *LogFile) EncodeEntry(e *db.Entry, buf *bytes.Buffer, offset uint32) (int, error) {
	h := db.Header{
...
func (lf *LogFile) DecodeEntry(buf []byte, offset uint32) (*db.Entry, error) {
	var h db.Header
	hlen := h.Decode(buf)
	kv := buf[hlen:]
	e := &db.Entry{
